...
export const HeroCarousel: React.FC<HeroCarouselProps> = ({ events, setShowLoginModal }) => {
    ...
    <>
      <div className="relative rounded-xl overflow-hidden mb-8">
        <div className="relative h-[400px] md:h-[500px]">
          {events.map((event, index) => (
            <div
              key={event.id}
              className={`absolute inset-0 transition-opacity duration-500 ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'
                }`}
            >
              <ImageWithFallback
                src={event.image}
                ...
                <div className='w-full flex flex-row justify-between items-center'>
                  ...
                  <div>
                    <ImageWithFallback
                      src={event.image}
                      alt={event.title}
                      className="h-[400px] object-cover brightness-50"
                    />
                  </div>
                </div>
              </div>
      ...
      </div>
      {
        ...
        <EventModal event={selectedEvent}
          ...
          open={!!selectedEvent}
          onOpenChange={(open: any) => { if (!open) setSelectedEvent(null) }}
        />
      }
    </>
  );
